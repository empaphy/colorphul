...
use Empaphy\Colorphul\Apca\SA98G;
...
//////////  APCA CALCULATION FUNCTIONS \/////////////////////////////////////
     ...
     *                         places.
     * @return ($places is 0 ? string : float)
     */
    public static function APCAcontrast(float $txtY, float $bgY, int $places = -1): float|string
    {
        $icp = [0.0, 1.1];  // input range clamp / input error check

        if (
            INF === $txtY || INF === $bgY || min($txtY, $bgY) < $icp[0]
            || max($txtY, $bgY) > $icp[1]
        ) {
        ...
        if (abs($bgY - $txtY) < SA98G::deltaYmin) {
            return 0.0;
        }
     ...
     * @param  float      $knownY
     * @param  'bg'|'background'|'txt'|'text'  $knownType
     * @param  'hex'|'color'|'Y'|'y'           $returnAs
     * @return ($returnAs is 'color' ? Rgb : string|float)|false
     *
     * @noinspection PhpUnused*@deprecated SOON
     */
    public static function reverseAPCA(
        float $contrast = 0,
        float $knownY = 1.0,
        string $knownType = 'bg',
        string $returnAs = 'hex'
        ...
        $unknownY = ($unknownY > SA98G::blkThrs) ? $unknownY : (pow(
                    (($unknownY + SA98G::mOffsetIn) * SA98G::mFactor),
                    SA98G::getMExp()
     ...
     */
    public static function calcAPCA(
        ...
        if ($hasAlpha) {
            $txClr = self::alphaBlend($txClr, $bgClr, $round);
        }

        $lc = self::APCAcontrast(self::sRGBtoY($txClr), self::sRGBtoY($bgClr), $places);
     ...
     * @noinspection PhpUnused
     */
    public static function fontLookupAPCA(int|float $contrast, int $places = 2): array
    {
        /**
...
//////////  LUMINANCE CONVERTERS  |//////////////////////////////////////////
     ...
     * @return float
     */
    public static function sRGBtoY(ColorInterface $rgb): float
    {
        /////   APCA   0.0.98G - 4g - W3 Compatible Constants   ////////////////////
        /*
     ...
     * @noinspection PhpUnused
     */
    public static function displayP3toY(P3 $rgb): float
    {
        /////   APCA   0.0.98G - 4g - W3 Compatible Constants   ////////////////////
     ...
     * @noinspection PhpUnused
     */
    public static function adobeRGBtoY(Rgb $rgb): float
    {
...
//////////  UTILITIES  \///////////////////////////////////////////////////
     ...
     * @return Rgb
     */
    public static function alphaBlend(Rgb $rgbaFG, Rgb $rgbBG, bool $round = true): Rgb
    {
